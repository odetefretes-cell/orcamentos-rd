...
function crmGerarOpcoes(db, o, d, cat){
  ...
  // São Bernardo → Marituba → Manaus (R$ 4.100) nunca era montada, enquanto São Paulo →
  // São Bernardo → Marituba → Manaus (R$ 4.600) era: saindo de São Paulo a lista é curta e
  // o trecho bom cabia. O motor enxergava o trecho ao CHEGAR em SBC, não ao PARTIR dele.
  // `custo[dN]` é o menor custo daquele ponto até o destino (Dijkstra, sem corte), então
  // `valor + custo[dN]` é o total da combinação que o trecho geraria: ordenar por ele faz o
  // corte guardar as MELHORES em vez das primeiras. Quando o corte não morde, o conjunto
  // gerado é o mesmo de antes — só a ordem de construção muda, e no fim tudo é reordenado
  // por preço. Nada de `saemDaOrigem.length=0`: foi esse padrão que sumiu com as diretas
  // em 04/09; aqui montamos uma lista nova e deixamos a original intacta.
  const _estOrig = a => (a.valor==null?Infinity:a.valor) + (custo[a.dN]==null?Infinity:custo[a.dN]);
  const _jaVi = new Set();
  const saidas = saemDaOrigem
    .filter(a=>{ const k=a.dN+'|'+normTxt(a.transportadora)+'|'+a.valor; if(_jaVi.has(k)) return false; _jaVi.add(k); return true; })
    .sort((x,y)=>_estOrig(x)-_estOrig(y));
...
function crmColetarOpcoes(db, coords, oR, dR, cat, oRefCo, dRefCo){
  ...
  // O CORTE ESCOLHE PELAS MELHORES, NÃO PELA ORDEM DE EXIBIÇÃO. Este `slice(60)` vinha
  // depois de um sort que põe DIRETAS primeiro — o que serve à tela, não à qualidade.
  // Guarda as 60 MAIS BARATAS (empate → embarque mais perto da origem real) e só então
  // aplica a ordem de exibição sobre as que sobraram.
  //
  // ⚠️ MEDIDO EM 11/09/2026: esta mudança NÃO ALTERA NENHUM PREÇO na tabela atual.
  // 576 pares de regressão: 576 iguais. 629 cotações instrumentadas: o corte é atingido
  // em 586 delas (até 541 opções), e o preço final difere em 0. O motivo: a ordem de
  // exibição põe as diretas primeiro JÁ ORDENADAS POR PREÇO, então a mais barata de todas
  // cai na 1ª posição e nunca é cortada. Para o corte descartar a melhor seria preciso
  // haver >60 diretas E uma combinação com transbordo mais barata que todas elas —
  // não ocorre hoje. Mantida por ser a ordem logicamente correta (mesma lição do corte
  // de 500), mas é DEFESA, não conserto: não creditar ganho de preço a ela.
  // Uma versão anterior deste comentário atribuía a ela o caso Nossa Senhora do Socorro
  // → Porto Alegre (R$ 2.375 vs R$ 2.555). Foi verificado: NÃO se reproduz — aquele caso
  // dá o mesmo resultado com e sem esta mudança. O ganho era das correções 1-3.
