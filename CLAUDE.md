...
21. **Conversões do Google Ads gravadas na PLANILHA, sem CSV (12/09/2026) — `integracao/conversoes-sheets.js`:** o Google aposentou o upload manual de arquivo (Metas → Uploads só redireciona para a Central de dados). O que ficou é uma planilha no Drive conectada ao Ads — **`OBS - Conversoes Google Ads (frete fechado)`**, id `1kaDe2x6UqnPOpiJ1ZeXF9B0T5rbK83_VwsgvSzKYTQg`, dona `odetefretes@gmail.com` — que o Google relê sozinho **todo dia entre 04:00 e 05:00** (Brasília). O job escreve nela.
   ...
   - 🐞 **INCIDENTE 14/09/2026 (causa a confirmar no log da VPS):** o 1º frete real após a virada (Bruno Felippello, fechado 09:04, R$ 1.851,49, com gclid) **não entrou** na planilha até 09:30 e ficou sem carimbo; o modal de CSV o via como pendente. A lógica de seleção é **idêntica** entre o app e o job (conferido linha a linha), então não é regra — é o ciclo das :17 que não rodou ou falhou. ⚠️ **O ciclo era silencioso quando não tinha nada a escrever** (`if (r.escritos || r.candidatos)`), logo "rodou e achou 0" e "não rodou" davam o mesmo log: nenhum. E o teste de 12/09 rodou a CLI **como root**, enquanto o serviço roda **como obsrobo** — permissão da chave (`chmod 600` + `chown`) é a 1ª suspeita, seguida da variável no ambiente do PM2.
