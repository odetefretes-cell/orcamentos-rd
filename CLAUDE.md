...
21. **Conversões do Google Ads gravadas na PLANILHA, sem CSV (12/09/2026) — `integracao/conversoes-sheets.js`:** o Google aposentou o upload manual de arquivo (Metas → Uploads só redireciona para a Central de dados). O que ficou é uma planilha no Drive conectada ao Ads — **`OBS - Conversoes Google Ads (frete fechado)`**, id `1kaDe2x6UqnPOpiJ1ZeXF9B0T5rbK83_VwsgvSzKYTQg`, dona `odetefretes@gmail.com` — que o Google relê sozinho **todo dia entre 04:00 e 05:00** (Brasília). O job escreve nela.
   ...
   - Testado sem rede (31 verificações): regras, fuso com o relógio em UTC, formato, assinatura do JWT conferida com a chave pública, retry, carimbo só após sucesso, abort por cabeçalho errado, dry-run e ausência de chave. **Não testado contra a planilha real** — o ambiente do Claude Code não alcança `googleapis.com` e não há chave aqui; o teste real é o roteiro do fim deste item.
   - As 7 conversões que já estavam na planilha foram carimbadas à mão em 12/09 — o job não as reescreve.
   - **Roteiro do teste real (Luiz):** lead de teste com gclid falso `TESTE_PLANILHA_SET2026` e R$ 100 → Venda Fechada → `node conversoes-sheets.js --once` → conferir a linha e o carimbo → rodar de novo e confirmar que não duplica → **apagar a linha de teste antes das 04:00** (gclid falso é rejeitado pelo Google, mas evita erro na importação).
