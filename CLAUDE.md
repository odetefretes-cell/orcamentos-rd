...
11b. **Integração fiscal OPHOS — `obs-fiscal-service/` (retomado 11/09/2026; NÃO confundir com o item 11):** há DOIS esforços fiscais no repositório e eles são diferentes:
   - **`fiscal/` + serviço `obs-fiscal` (porta 3003, Focus NFe)** — o do item 11, pausado.
   - **`obs-fiscal-service/`** — o projeto OPHOS, que é o vivo. Node CommonJS puro, sem package.json. `docs/ARQUITETURA.md` aprovado pelo Luiz, `docs/regras-negocio.md` (§6) e `src/validators/` (CT-e, CIOT, MDF-e) com **16 testes passando**.
   - ⚠️ **Rodar os testes com `node --test`** (na pasta do serviço). **`node --test test/` QUEBRA no Node 22** com `MODULE_NOT_FOUND` — parece defeito do código e não é. `npm test` não existe (sem package.json). O README dizia as duas coisas erradas até 11/09.
   - **Canal confirmado (29–31/08): API REST**, `developer.ophos.com.br/?url=/api/cte.json` e `/api/mdfe.json`, documentação **pública**. A §2 da arquitetura descrevia o **Integrador Windows com pastas e TXT** e afirmava que o TXT "cobre CIOT" — **era suposição e estava errado**; reescrita em 11/09.
   - **Escopo real da API:** ✅ CT-e, MDF-e, eventos (cancelamento, carta de correção, encerramento) e download de PDF/XML. ⛔ **CIOT e GNRE ficam de fora** ("NÃO temos integração ainda" / "não emitimos GNRE") — seguem manuais. ❓ DC-e ambíguo ("possível emitir junto com o CTe"). ⚠️ Averbação só volta se a apólice for AT&M, Porto Seguro, ELT Seguro ou Smart Load.
   - **Sem webhook:** duas chamadas (emitir / consultar). Exige fila, reconsulta com espera progressiva e estado "aguardando autorização" visível ao operador. Sem limite de chamadas. **A numeração passa a ser nossa** — tela + API ao mesmo tempo fura a série.
   - **Custo — DECIDIDO (11/09): faixa de 100.** R$ 340 de ativação + **R$ 344,90/mês** no volume real (**122 CT-e e 110 MDF-e**/mês; o nº de MDF-e veio do Luiz em 11/09). A faixa de 200 (R$ 481,17 fixos) só compensaria a partir de **215 CT-e/mês**, quase o dobro. Economia de R$ 136,27/mês; no pior mês já registrado (142 CT-e) ainda economiza R$ 102,51. Sem fidelidade → dá para subir de faixa depois.
   - ⚠️ **A conta "163 documentos/mês" do artefato anterior era ambígua** e induzia ao erro: supunha CT-e e MDF-e no mesmo número (328 documentos no total). Com os volumes reais a virada é 215 CT-e.
   - **Plano WEB atual: R$ 346,00/mês** (informado pelo Luiz 11/09). Convivência dos dois caminhos = **R$ 690,90/mês**; R$ 1.030,90 no 1º mês com a ativação; **R$ 8.630,75 no ano 1**.
   - ✅ **DC-e ENTRA** (confirmado pelo Luiz 11/09) — some da lista de dúvidas comerciais. Resta só a checagem técnica de como (endpoint próprio ou junto do CT-e), que é assunto de homologação.
   - **O que trava hoje NÃO é técnico:** a proposta de 31/08 segue **sem resposta da OBS**, e o cronograma só vem depois do "ok". Das 4 dúvidas que podiam mudar a decisão, **3 foram respondidas em 11/09**; resta **uma**: a averbação.
   - ✅ **DECISÃO DO LUIZ (11/09): CONTRATAR a faixa de 100.** A averbação fica para depois do contrato. Justificativa: sem fidelidade nem multa (risco = 1 mensalidade), a averbação não muda plano nem preço, o grosso do valor (CT-e, MDF-e, eventos, downloads) não depende dela, e assinar libera a homologação — que é onde a dúvida se resolve testando.
   - ⛔ **CONDIÇÃO DA DECISÃO — NÃO migrar a emissão REAL para a API antes de resolver a averbação.** Desenvolver e testar em homologação à vontade; o primeiro CT-e de verdade pela API só depois de saber como o nº de averbação entra. O número precisa estar no documento **ANTES da transmissão** (depois de autorizado não entra mais) — hoje o operador digita na tela. Cortar a emissão para a API sem isso faria CT-e sair **sem averbação**, pior que a situação atual.
   - Ao fechar, pedir à Angélica: **cronograma de implantação** (ela só envia após o "ok") e **credenciais de homologação + em quantos dias úteis saem** (pergunta que ficou sem resposta em 31/08).

   **AVERBAÇÃO — provavelmente NÃO há nada a contratar (levantado 11/09):** o Luiz pediu para "contratar um provedor homologado". Antes de gastar, dois achados:
   - `fiscal/PROJETO-CTE-MDFE-AVERBACAO.md` (22/08) já registrava que no caso da **AT&M o custo "geralmente está incluído na relação seguradora/corretora"** — não é fornecedor que se contrata à parte, é serviço que a **corretora habilita na apólice existente**.
   - A **AT&M atende 95% das seguradoras** do país e detém 95% do mercado de averbação eletrônica de cargas. A chance de a apólice **Allianz já passar por ela** é altíssima — faltariam só as credenciais (usuário, senha e código ATM vinculados à apólice).
   - O mesmo documento levanta um 3º caminho: **algumas seguradoras averbam sozinhas**, capturando o CT-e direto da SEFAZ. Se a Allianz for assim, o sistema só **lê e registra** o número — não há integração a fazer.
   - **Ação certa: ligar para a CORRETORA (não para a OPHOS)** e perguntar: (1) qual a averbadora da apólice `517720243Y540000497` — é a AT&M? (2) credenciais de integração; (3) a apólice já averba automático pela SEFAZ ou é preciso enviar o XML?
   - 🔸 **PERGUNTA QUE PODE SER GRAVE:** hoje o nosso sistema guarda um **contador sequencial PRÓPRIO** de averbação (em **757**, `src/domain/contadores.js`) e o operacional digita esse número em cada carga. Se a averbação é declaração à seguradora, o número deveria **vir dela**. Pode estar certo (controle interno + averbação automática por trás), mas **confirmar com a corretora** — se o número deveria vir da averbadora, isso é mais urgente que a integração: significaria cargas rodando sem averbação válida.
   - 🔸 **Contexto original (a resposta da OPHOS em 31/08):** a OPHOS só devolve o nº de averbação pela API se for por **AT&M, Porto Seguro, ELT Seguro ou Smart Load**. Nossa apólice cadastrada é **Allianz** (`517720243Y540000497`, em `src/domain/contadores.js`), que não está na lista — mas seguradora e averbadora costumam ser empresas diferentes, então pode passar por uma das quatro. Perguntar à OPHOS/corretora **por qual averbadora a apólice é processada** e **se dá para enviar o número que já geramos** (o contador é nosso, hoje em 757), sem depender da averbação automática. Pesa porque o operacional inclui **um nº por carga, ANTES de transmitir** — depois de autorizado não entra mais.
   - Página de decisão para encaminhar à OPHOS (plano, conta e a pergunta que falta): https://claude.ai/code/artifact/33d5c086-df75-40e8-82bb-f687e7252943
   - ⚠️ **As specs não podem ser baixadas do ambiente do Claude Code** — a rede de saída bloqueia `developer.ophos.com.br` e `www.ophos.com.br`. Baixar **pela VPS** (saída livre, como foi com o Conta Azul) ou pelo navegador, e colocar em `obs-fiscal-service/docs/ophos-layouts/`.
   - Artefato com as 18 respostas da OPHOS, as propostas e a linha do tempo: https://claude.ai/code/artifact/66469672-ca9a-4928-952d-6e7c5c128c30
   - Driver ativo hoje: a skill de navegador `ophos-obs-documentos-fiscais` (emite na tela do OPHOS). Validação/regras/orquestração não mudam quando a API entrar — troca só o motor de transmissão.
...
16. **Relatório do comercial de 11/09/2026 — itens de tabela aplicados + o bug do recebimento.**
   ...
   **Taxa de recebimento de base: falta de cadastro entrava como ZERO — CORRIGIDO:** o orçamento oficial do cliente Carlos Eduardo (Salvador → SBC → Itajaí) saiu com Recebimento de R$ 100. O comercial reportou como "somou só uma das duas bases", mas o somatório está certo: Itajaí tem R$ 100 cadastrado e **Salvador está com `recebimento: null`**. Base sem cadastro contribuía 0 em silêncio, e o oficial saía abaixo do custo.
   ...
   - **APLICADO EM PRODUÇÃO 11/09/2026 17:41** (489 → **490 rotas**), backup em `/root/backup-tabela-2026-09-11T17-41-37-745Z.json`. Com isso os 6 itens de tabela do relatório estão fechados.
   - ⚠️ **Armadilha ao pegar um arquivo da branch na VPS:** `git fetch origin <branch>` neste clone atualiza só o `FETCH_HEAD`, **não** a ref `origin/<branch>` — o `git show origin/<branch>:arquivo` seguinte lê uma versão ANTIGA e o dry-run diz "nada a mudar" sem erro nenhum. Aconteceu em 11/09 e custou uma rodada. Usar **`git show FETCH_HEAD:<arquivo>`** logo após o fetch. (Os `deploy-*.sh` não sofrem disso: usam o refspec explícito `$BRANCH:refs/remotes/origin/$BRANCH`.)
...
17. **Moto acima de 300cc no Nordeste ia por São Paulo — faixa sem preço, não bug do motor (14/09/2026):** o lead do Oseias (João Pessoa → Fortaleza, moto acima de 300cc) saía FMartins JP → SBC → Fortaleza, R$ 2.400, dois embarques. Na VPS, com a tabela real (490 rotas) e o motor publicado, Carro Passeio dava Diniz direto R$ 750 e a volta Anilton R$ 800 — então o motor estava certo. A diferença era a **categoria**: Anilton, Diniz, Expresso Car e Toninho só tinham preço em Carro Passeio, Carro Grande e Moto até 300cc. Sem preço em "Moto até 700cc"/"Moto acima de 700cc" a rota deles nem entra (aparece "Toninho · s/ preço"), e sobra a FMartins, cujo único caminho passa por SBC. **Diagnóstico que fecha rápido: rodar o par nas 5 categorias** — foi isso que revelou (ver o comando do item 13).
   - ✅ **CONFIRMADO PELO LUIZ (14/09): nessas transportadoras, moto acima de 300cc paga o preço do Carro Passeio.** Aplicado pelo bloco NOVO **`COPIAR_CATEGORIA`** do `atualizar-tabela-precos.mjs` (faixa VAZIA recebe o preço de outra faixa da mesma rota; faixa já preenchida não é tocada): Anilton, Diniz e Expresso Car, `m700` e `m700+` ← `p`. Na cópia local: 42 valores; JP → Fortaleza moto grande passou a Diniz direto R$ 750. **APLICADO EM PRODUÇÃO 14/09/2026 12:24** (mesmos 42 valores, 490 rotas), backup em `/root/backup-tabela-2026-09-14T15-24-11-143Z.json`.
   - ⚠️ **É regra por transportadora, NÃO fallback no motor.** FMartins tem preço próprio de moto grande (menor que o de carro), e uma transportadora que não leva moto grande não pode ganhar preço inventado. Toninho ficou de fora (só carro cadastrado, não confirmado).
   - Antes disso o diagnóstico passou por dois falsos suspeitos: navegador com app antigo (o deploy-app.sh de 14/09 11:56 e o Ctrl+Shift+R não mudaram nada) e tabela de produção diferente da cópia (não era). Lição: quando o motor na VPS dá certo e a tela dá errado com a mesma tabela, a diferença está no **input** (categoria, quantidade, cidade), não no código.
...
19. **Apuração de perdas — relatório, motivos e lote (11/09/2026):**
   ...
   **O relatório "📞 Perdas para apurar"** (`crmRelatorioPerdasApurar` no `index.html`) lista os leads perdidos em que NINGUÉM chegou a falar com o cliente, com telefone clicável, para o operador ligar e descobrir o motivo verdadeiro.
   ...
   - **Agrupado por OPERADOR (pedido do Luiz, 14/09/2026):** o relatório sai em blocos por vendedor (cabeçalho com quantidade, novos e valor; numeração e total próprios), com botões no alto — "Todos (N)" e um por vendedor — que filtram a tela. **Impressão em A4 paisagem** (`@page`), cabeçalho da tabela repetido a cada folha, linha nunca partida; em "Todos" cada operador sai numa folha (classe `quebra` aplicada por JS só nas seções visíveis menos a última, para não sair folha em branco). Conferido em Chromium headless com dados de teste: 1 operador = 1 folha; 3 operadores = 3 folhas. A coluna "Vendedor / motivo atual" virou só "Motivo atual" (o vendedor está no cabeçalho do bloco).
   - 🐞 **DERRUBEI O APP em 14/09 por 20 minutos com isso — registrar para não repetir:** o HTML do relatório é montado num template literal dentro do script do `index.html`, e eu escrevi um `<script>…</script>` literal nele. **O navegador encerra o script principal no primeiro `</script>` que vê, mesmo dentro de string JS.** O app inteiro parou de carregar (tela vazia, logo quebrado, "sistema travou"). O `new Function()` do bloco extraído NÃO pega isso — só o HTML inteiro pega. Correção: escrever `<\/script>` dentro do template. **Verificação obrigatória antes de qualquer deploy-app: contar os scripts inline do `index.html` e checar a sintaxe de cada um** — `node -e "const s=require('fs').readFileSync('index.html','utf8');const re=/<script(?![^>]*src=)[^>]*>([\s\S]*?)<\/script>/gi;let m,i=0,e=0;while((m=re.exec(s))){i++;try{new Function(m[1])}catch(x){e++;console.log(x.message)}}console.log('scripts',i,'erros',e)"` — hoje são **3 scripts, 0 erros**; qualquer outra contagem é quebra.
...
20. **Parcelamento no cartão dentro do orçamento (11/09/2026):** o orçamento oficial dizia só *"Cartão de crédito em até 12x (com acréscimo)"*, sem valor — e no Financeiro o campo **"Valor c/ taxas do cartão" era DIGITADO À MÃO**, sem cálculo. Errar a conta ali significa a OBS receber menos que o frete.
   - **`CRM_TAXA_REDE`** (no `index.html`) traz a taxa TOTAL da Rede = MDR + **antecipação automática** (dinheiro em 1 dia útil): 4,19% à vista → 13,24% em 12x → 17,86% em 18x. Modalidade Link de Pagamento / e.Rede, Mastercard e Visa. **`CRM_CARTAO_MARGEM = 0.80` p.p.** cobre Elo e Amex, que custam sempre isso a mais.
   - ⚠️ **A conta NÃO é "valor + taxa%".** A taxa incide sobre o total COBRADO, então é `1/(1−taxa)`; somar por cima deixa a OBS recebendo menos. Conferido: frete R$ 3.000 em 12x → cobra **R$ 3.490,08**, a Rede fica com R$ 490,01, a OBS recebe **R$ 3.000,07**. A parcela é arredondada para CIMA, então o erro é sempre a favor.
   - **Parcelas exibidas (`CRM_CARTAO_ANCORAS`, pedido do Luiz 11/09): 1, 2, 3, 4, 5, 6 e 12.** O salto de 6 para 12 é de propósito — 7x a 11x não acrescentam decisão ao cliente e só alongariam a tabela. O **maior prazo possível entra sempre**, mesmo fora da lista: frete de R$ 1.200 mostra até 9x (no 12x a parcela cairia abaixo de R$ 150) e frete de R$ 13.000 mostra até 11x (o 12x estouraria o teto do link).
   - Sai como **bloco destacado** (moldura azul da OBS + tabela de parcelas), não como linha corrida — em papel, 7 valores separados por ponto viram ruído.
   - ✅ **DOIS parcelamentos quando há coleta/entrega (pedido do Luiz 11/09): base a base E porta a porta.** São preços diferentes, e a taxa do cartão incide sobre o total COBRADO — mostrar só um levaria o cliente a fechar porta a porta achando que paga a parcela do base a base. Ex.: base R$ 3.000 → 12x de R$ 290,84; porta a porta R$ 3.700 → 12x de R$ 358,70. A linha do Pix também mostra os dois valores. Sem coleta/entrega, cai no bloco único de antes.
   - ⚠️ **Teto de 12x** (`CRM_CARTAO_MAX`): o Link de Pagamento do painel trava aí. Os **18x exigem a integração e.Rede** — as taxas até 21x já estão contratadas, falta habilitar. Quando habilitar, é só subir a constante.
   - Guardas: **teto de R$ 15.000 por link** da Rede e **parcela mínima de R$ 150** (regra comercial da OBS). Frete de R$ 13.000 só fecha em 11x; de R$ 300, em 2x.
   - ⚠️ **RECONFERIR A CADA TRIMESTRE** no painel userede: a taxa de **antecipação é renegociável** e é ela que pesa — dos 17,86% do 18x, **14,44 pontos são antecipação**. Se um dia a OBS puder pagar o prestador com prazo, desligar a antecipação nas vendas longas derruba o custo para perto de 3,42%.
   - **Base legal:** a Lei 13.455/2017 autoriza preço diferente por instrumento de pagamento, **desde que informado de forma ostensiva** — por isso o orçamento passou a trazer o preço do Pix e o do cartão lado a lado. ⚠️ **Nunca escrever "sem juros"**: como o total muda por prazo, o correto é "valores já com a taxa da operadora inclusa".
   - ⚠️ **A nota sai pelo valor COBRADO**, não pelo frete — o CT-e/NFS-e vão pelo total do cartão. Num frete de R$ 3.000 em 18x isso é ~R$ 76 de imposto a 11% saindo da margem. O artefato tem campo de alíquota do Simples para embutir isso; **NÃO foi implementado no CRM** — decidir se o acréscimo deve cobrir o imposto também.
   - ⚠️ **Venda por link é cartão não presente:** em contestação, o prejuízo é do estabelecimento. Exigir contrato assinado e documento antes de liberar a coleta.
   - Artefato com a tabela interativa, a composição da taxa e as travas do plano: https://claude.ai/code/artifact/ba18c739-afaf-47a9-81cb-44915964295a
   - **NÃO feito:** o campo manual do Financeiro continua manual. Fechar o ciclo (o valor que o cliente viu virar a cobrança) é o próximo passo.

   **Como gerar o link de cobrança sozinho — 3 caminhos (artefato revisto em 11/09):**
   1. **Ligar para a Rede e PEDIR** — liberar mais de 12x e teto acima de R$ 15.000 no Link de Pagamento. **Custo zero, sem código, e faz sentido antes de escrever qualquer linha:** as taxas até 21x JÁ estão contratadas no plano, então o 12x parece limite do produto, não do contrato. Continua manual (alguém digita cada link no painel).
   2. **e.Rede direto** (`api.userede.com.br/erede/v1/transactions`, Basic com PV `65343450` + token, `installments`, 3DS 2.0, SDK Node oficial) — ⛔ **NÃO RECOMENDADO: o número do cartão passaria pelo nosso sistema, o que joga a OBS para dentro do escopo PCI-DSS.** Peso regulatório desproporcional para uma transportadora.
   3. **Gateway sobre a Rede** ✅ **recomendado** — o contrato, as taxas e o dinheiro continuam da OBS; quem hospeda a tela do cartão e carrega o PCI é o gateway. Entrega link por API, 3DS e **webhook de pagamento aprovado** (que é o que permite dar baixa no frete e liberar a coleta sozinho). Custo: mensalidade. O iPag anuncia integração com Itaú/Rede — cotar dois ou três.
   - **Perguntas para a ligação:** dá para liberar mais de 12x no link, e por quê não, se a tabela vai até 21x? Qual o limite real de parcelas na e.Rede do PV (o manual público diz 2 a 12, o plano mostra 21)? O teto de R$ 15.000 pode subir (cegonha com 2–3 carros passa disso)? A antecipação vale igual por e.Rede e por gateway parceiro? 3DS e antifraude entram na taxa ou são à parte? Emitir token de integração e acesso ao sandbox.
   - **Tetos conferidos** (batem com a nossa implementação): frete máximo em **12x = R$ 12.894**; em 18x = R$ 12.201. Acima disso, dois links ou Pix.
   - **O passo 3 da sequência do artefato — "ligar a tabela de coeficientes no orçamento" — JÁ ESTÁ FEITO** e não dependia de ninguém. Os passos seguintes (sandbox, piloto com um vendedor, conciliação por webhook) dependem da ligação e da escolha do gateway.

21. **Conversões do Google Ads gravadas na PLANILHA, sem CSV (12/09/2026) — `integracao/conversoes-sheets.js`:** o Google aposentou o upload manual de arquivo (Metas → Uploads só redireciona para a Central de dados). O que ficou é uma planilha no Drive conectada ao Ads — **`OBS - Conversoes Google Ads (frete fechado)`**, id `1kaDe2x6UqnPOpiJ1ZeXF9B0T5rbK83_VwsgvSzKYTQg`, dona `odetefretes@gmail.com` — que o Google relê sozinho **todo dia entre 04:00 e 05:00** (Brasília). O job escreve nela.
   - **Como roda:** cron **`17 * * * *`** no orquestrador (item 8), de hora em hora. É um job varrendo os fechados, NÃO um gancho no clique de "Venda Fechada" — menos invasivo no Kanban, e o Google só lê de madrugada mesmo. Avulso na VPS: `cd /opt/obs-automacao/integracao && node conversoes-sheets.js --dry-run` (mostra o que escreveria) ou `--once` (escreve).
   - **Regras (espelham `crmColetarConversoesAds` do botão de CSV, que segue como plano B):** etapa `fechado` · tem `gclid`/`gbraid`/`wbraid` (sem clique pula calado) · total da composição > 0 · tem entrada `convertido` na timeline (única fonte com HORA) e não está no futuro · **ainda sem `conversao_exportada_em`** · clique com ≤ 90 dias (o Ads recusa mais velho).
   - **Anti-duplicação = o MESMO carimbo `conversao_exportada_em` do botão de CSV**, por isso os dois caminhos não brigam. Gravado **só depois** de a API confirmar o `append`; se falhar, o lead fica sem carimbo e volta na próxima hora. ⚠️ O Google relê a planilha INTEIRA toda madrugada — a mesma venda com dois horários conta DUAS vezes, e conversão importada não sai fácil. **Nunca apagar linha** (some do histórico); se crescer demais, mover >120 dias para uma aba de arquivo.
   - **Formato, literal:** `Google Click ID` (gclid cru) · `Conversion Name` = **`OBS - Frete fechado`** (letra por letra) · `Conversion Time` = `AAAA-MM-DD HH:MM:SS` em **America/Sao_Paulo** (o servidor roda em UTC — formatado com `Intl`, senão sairia 3h errado) · `Conversion Value` = `1922.00` (ponto, 2 casas, sem R$/milhar) · `BRL`. Escrita **`valueInputOption=RAW`** para o Sheets não converter data em número de série nem valor em moeda.
   - ⚠️ **O CABEÇALHO (linha 1) está amarrado ao mapeamento dentro do Google Ads** — renomear/reordenar/remover coluna para a importação em silêncio. O job **confere `A1:E1` antes de escrever e aborta se não bater**; `values.append` só acrescenta após a última linha com dados, nunca sobrescreve.
   - **Acesso:** service account + Google Sheets API, JWT RS256 assinado com o `crypto` nativo — **sem SDK, sem dependência nova** (Node 22 tem `fetch`). Chave em **`/etc/obs-automacao/google-sa.json`** (chmod 600, dono obsrobo), apontada por `GOOGLE_SA_KEY_FILE` no `.env`; a planilha precisa estar compartilhada com o `client_email` como **Editor**. **Sem a variável o job só avisa no log e não faz nada.**
   - **Retry:** 429/5xx/rede → 3 tentativas com 2 s e 4 s; 401/403/404 falha na hora (chave errada / sem acesso à planilha / id errado — repetir não ajuda). O ciclo nunca derruba o orquestrador. Log por linha: nome, gclid truncado, valor, hora, faixa gravada.
   - ⚠️ **Rodar a CLI avulsa exige `vps/node_modules` no NODE_PATH** (o `dotenv` está lá, não em `integracao/`) — o próprio arquivo cuida disso; sem isso o `.env` não carregava e o job dizia "sem chave" sem explicar.
   - Testado sem rede (31 verificações): regras, fuso com o relógio em UTC, formato, assinatura do JWT conferida com a chave pública, retry, carimbo só após sucesso, abort por cabeçalho errado, dry-run e ausência de chave.
   - ✅ **VALIDADO EM PRODUÇÃO (12/09/2026):** service account `obs-conversoes-ads@obs-fretes.iam.gserviceaccount.com` (projeto `obs-fretes`, Sheets API ativada, sem papel no projeto — o acesso vem só do compartilhamento da planilha como Editor). Lead de teste `TESTE_PLANILHA_SET2026` (R$ 100, fechado 14:30Z) → `--once` escreveu em `A9:E9` com `2026-09-12 11:30:00` e `100.00`; segundo `--once` devolveu 0 candidatos. Linha de teste apagada da planilha e lead removido do banco.
   - ⚠️ **O dry-run NÃO chama o Google** (não troca token nem lê a planilha) — só prova que a chave carregou e o banco respondeu. Chave errada e falta de compartilhamento só aparecem no `--once`.
   - ⚠️ **Armadilhas do setup vistas em 12/09:** (1) o projeto tem 4 contas de serviço; a planilha foi compartilhada primeiro com a **`obs-fretes@appspot.gserviceaccount.com`** (App Engine), que NÃO é a da chave — conferir que o e-mail compartilhado é o `client_email` do JSON. (2) A conta nova não aparece na tela **IAM** (sem papel), só em **Contas de serviço**. (3) O `nano` da chave ficou sem salvar na 1ª vez → `ENOENT` no job; conferir com `ls -l` (~2.360 bytes). (4) O terminal web da Hostinger **traduz a saída** ("garfo", "verdadeiro", "INSERIR 0 1", e `100.00` vira `100,00`) — o valor real está na planilha.
   - ⚠️ **O campo GCLID da ficha do lead é só leitura de propósito** (vem do formulário do site). Para um lead de teste, inserir direto no Postgres: `INSERT INTO crm_leads (id, data, updated_at) VALUES ('lead_teste_planilha', '{"nome":…,"etapa":"fechado","gclid":…,"composicao":[{"valor":"100","ativo":true}],"timeline":[…,{"tipo":"convertido","data":"<ISO no passado>"}]}'::jsonb, now())` e apagar com `DELETE FROM crm_leads WHERE id='lead_teste_planilha'`. O job precisa de `timeline[].tipo='convertido'` com data no passado e `composicao` ativa > 0.
   - As 7 conversões que já estavam na planilha foram carimbadas à mão em 12/09 — o job não as reescreve. ⚠️ O botão de CSV **carimba ao baixar**: um clique nele tira o lead da fila do job. Se o CSV trouxer venda que não está na planilha, colar a linha à mão.
   - ✅ **"INCIDENTE" 14/09/2026 — FALSO ALARME, o job funcionou:** um relatório afirmou que o 1º frete real (Bruno Felippello, fechado 09:04, R$ 1.851,49, com gclid) "não entrou na planilha até 09:30 e ficou sem carimbo". O log do PM2 mostra o contrário: `2026-09-14T09:17:04 [conversoes] ✔ Bruno Felippello (lead_wpp_66233546) … R$ 1.851,49 em 2026-09-14 09:04:00 → Sem título!A9:E9` e `ciclo: {"candidatos":1,"descartados":0,"escritos":1}`. A conferência foi feita **antes do ciclo das :17** ou com a planilha/CRM abertos sem recarregar (o modal de CSV lê a cópia local do navegador). **Latência de até 1 h entre "Venda Fechada" e a linha é comportamento esperado** — o Google só lê a planilha de madrugada. Lição: antes de abrir incidente do job, olhar `pm2 logs obs-automacao | grep conversoes` e conferir se já passou um :17.
   - ⚠️ Mesmo assim havia um defeito de observabilidade real: **o ciclo era silencioso quando não tinha nada a escrever** (`if (r.escritos || r.candidatos)`), logo "rodou e achou 0" e "não rodou" davam o mesmo log: nenhum. E o teste de 12/09 rodou a CLI **como root**, enquanto o serviço roda **como obsrobo** — confirmado em 14/09 que obsrobo lê a chave (`test -r` → LEGIVEL) e que o dry-run como obsrobo funciona.
   - **Reforço feito em 14/09:** (1) o orquestrador loga **uma linha por ciclo, sempre** (`[conversoes] ciclo cron em Xms: {...}`), com stack em erro; (2) no boot imprime `[conversoes] chave da service account: OK — <arquivo> (<client_email>), lida pelo usuário obsrobo` ou o erro (ENOENT/EACCES/JSON) — `diagnosticoChave()`; a CLI imprime a mesma linha; (3) **rodada 90 s após o boot**, além do cron — um deploy/restart não deixa frete esperando o próximo :17 e serve de teste do deploy. Comandos de diagnóstico na VPS, nesta ordem: `su - obsrobo -c "pm2 logs obs-automacao --lines 1000 --nostream" | grep -i conversoes` · `su - obsrobo -c "test -r /etc/obs-automacao/google-sa.json && echo LEGIVEL || echo SEM ACESSO"` · `grep -c GOOGLE_SA_KEY_FILE /etc/obs-automacao/.env` · `su - obsrobo -c "cd /opt/obs-automacao/integracao && node conversoes-sheets.js --dry-run"` (**como obsrobo**, não root).
...
15. **Rastreamento de anúncio (gclid) + armadilhas do ChatGuru (07/09/2026) — VALIDADO EM PRODUÇÃO com lead real:**
   ...
   **Fechamento da conversão offline (atualizado 12/09):** o botão de CSV existe (`crmBaixarExportAds`) mas o **Google aposentou o upload de arquivo** — o caminho vivo é a **planilha conectada ao Ads, alimentada pelo job `conversoes-sheets.js` (ver item 21)**. O botão fica como plano B; os dois usam o mesmo carimbo `conversao_exportada_em`. Ainda com o Luiz: a caixa de consentimento no Google Ads (declaração legal).
