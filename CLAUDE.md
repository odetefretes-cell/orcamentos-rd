...
9. **Fluxo de pagamento no Financeiro — fila de liberação (22/08/2026):**
   ...
   **Fase 2 (integrações):**
   ...
   - **Pendente (Fase 3):** baixa automática CA → sistema (robô consulta parcelas pagas; PIX pago → libera ao operacional sozinho; boleto → só baixa) e cartão baixado no sistema → **baixa no CA** (API de baixas — formato a descobrir testando, como nas outras).
